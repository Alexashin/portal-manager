...
def get_module_management_keyboard(module_id) -> ReplyKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="✏️ Редактировать название и описание", callback_data=f"edit_module_title_and_desc_{module_id}")],
        [InlineKeyboardButton(text="🗑 Удалить модуль", callback_data=f"delete_module_{module_id}")],
        [InlineKeyboardButton(text="📖 Управление уроками модуля", callback_data=f"manage_lessons_{module_id}")]
    ])
