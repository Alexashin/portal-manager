...
@ad_router.message(CommandStart(), RoleFilter("manager"))
async def cmd_start(message: Message) -> None:
    await message.answer('Старт менеджер')


# Старт создания модуля
@ad_router.message(RoleFilter("manager"), Command("create_module"))
...
@ad_router.message(ModuleCreation.waiting_for_module_title)
...
@ad_router.message(ModuleCreation.waiting_for_module_description)
...
# Ввод названия урока
@ad_router.message(ModuleCreation.waiting_for_lesson_title)
...
# Ввод текста урока
@ad_router.message(ModuleCreation.waiting_for_lesson_text)
...
# Загрузка файлов
@ad_router.message(ModuleCreation.waiting_for_lesson_files, F.document)
...
@ad_router.message(Command("skip"), ModuleCreation.waiting_for_lesson_files)
...
# Загрузка видео
@ad_router.message(ModuleCreation.waiting_for_lesson_video, F.video)
...
@ad_router.message(Command("skip"), ModuleCreation.waiting_for_lesson_video)
...
@ad_router.callback_query(F.data == "add_lesson")
...
# Завершение модуля
@ad_router.callback_query(F.data == "finish_module")
